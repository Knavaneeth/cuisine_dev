	...
	success: function (data) {
		...
		} else if(data.code==3)
		{
			...
			switch (action)
			{
				...
				//swal(data.msg);				 
				setTimeout(function () { GetTodaysOrder(); }, 3000);	
				break;
				...
				modalloader.css("display","none");
				hideAllModal();
				if(getNewPendingOrderTimeout!='')
				{
						clearTimeout(getNewPendingOrderTimeout);	
				}				
				getNewPendingOrderTimeout = setTimeout(function(){getNewPendingOrders();},20000);
				
				setTimeout(function () { GetTodaysOrder(); }, 8000);						 
				setTimeout(function () { getNewBooking(); }, 6000);						 
				...
				$("#new-booking-error").html(data.msg);
					setTimeout(function () { getAllBooking(); }, 4000);
				...
				$("#new-orders").html(' Waiting for order...');
					setTimeout(function () { getGetAllOrders(); }, 4000);
				break;
...
{ //new-orders
	...
	$.each( data, function( key, val ) {
		...
		if($.trim(val.status)=="decline") { val.status = "Declined"; }
		...
		htm+='<div class="media-body media-middle">';
		 ...
		 htm+='<div class="media-right media-middle">';
		   htm+='<div class="order-det">	<div class="order-amount"><b>'+parseFloat(val.bill_total).toFixed(2)+'</b></div>';
...
function displayDriverSummary(data,div_id)
{
	...
	{
	$.each( data , function( key, val ) {
		total += parseFloat(val.bill_total);
		htm+='<tr>';
		...
		htm+='<td>£ '+parseFloat(val.bill_total).toFixed(2)+'</td>'
		htm+='</tr>';
	});
...
{ //new-orders
	...
	$.each( data, function( key, val ) {
		...
		htm+='<div class="media-body media-middle">';
		 ...
		 htm+='<div class="media-right media-middle">';
		   htm+='<div class="order-det">	<div class="order-amount"><b>'+parseFloat(val.bill_total)+'</b></div>';
		   htm+='<div class="order-time">';
		   if (!empty(val.delivery_time)){
		     htm+='<i class="fa fa-clock-o fa-lg"></i>';
		     htm+=' <b>'+val.delivery_time+'</b> ';
...
{ //new-orders
	...
	createElement(div_id,htm);
	//setTimeout(function(){getNewPendingOrders();},4000);
	setTimeout(function () { GetTodaysOrder(); }, 4000);		
...
function displayNewOrders(data,div_id)
{
	...
	//	loader.show();
		...
		setTimeout(function () { GetTodaysOrder(); }, 6000);
		setTimeout(function () { getNewBooking(); }, 9000);
}
...
function acceptOrder()
{
   ...
   //kNavigator.pushPage("acceptOrderForm.html", options); PendingBooking
}
...
{	 	    
	...
	if($.trim(data.status)=="decline") { data.status = "Declined"; }
	var html='';
	var html='<ul class="view-table">';
     ...
     if (data.trans_type_raw=="delivery"){
	     ...
	     html+='<div class="row">';
			...
			//  html+='<ons-button modifier="quiet" onclick="viewLocationNew('+lat_lng+')" class="view-location">';
				var urls="'test_map.php?address="+address+"','_blank'"
     ...
     if (!empty(data.item)){
      ...
      if (!empty(data.free_details))
      {
	     	...
	     	{    	     			 
				    var size_words = '';  
				    var item_total = (parseInt(val.qty)*val.normal_price);							      
				    if(parseInt(val.discounted_price)!=0)
				    {					    	 
				    ...
				    if(val.free_type=="BOGO"&&val.multi_size_free=="true")
				    {				    	 
				    	var different_size_details = val.size_details.split("||");
				    	var different_price_details = val.price_details.split("||");				    	 
				    	$.each( different_size_details , function( different_size_details_key, different_size_details_val ) 
	     				{	 	     					 
	     					html+= '<li><div class="row"><div class="col-sm-7 fixed-col bold">'+val.qty+'x '+different_price_details[different_size_details_key]+' '+val.item_name+different_size_details_val+'</div><div class="col-sm-5 text-right">'+free_type+' £ '+parseFloat(different_price_details[different_size_details_key]).toFixed(2)+'</div></div></li>';
	     				});
				    }
				    else
				    {	    	 
				    	html+= '<li><div class="row"><div class="col-sm-7 fixed-col bold">'+val.qty+'x '+val.normal_price+' '+val.item_name+size_words+'</div><div class="col-sm-5 text-right">'+free_type+' £ '+item_total.toFixed(2)+'</div></div></li>';
				    }
        ...
        html+='<div class="col-sm-5 text-right font-bold">'+data.total.subtotal+'</div>';
        html+='</div>';
        html+='</li>';

        var all_items_total = parseFloat(data.total.subtotal.replace('£',''));
     
     //alert(data.discount_details.toSource());  
     if (!empty(data.discount_details))
     {
     	var discounted_percentage =   data.discount_details[0].discount_percentage +" % " ;
     	//alert(data.discount_details[0].discount_percentage);
     	//alert(data.discount_details[0].discount_price);
	    html+=TPLorderRow( getTrans("Discount",'discount') +" - "+ discounted_percentage , "("+ (Math.ceil(data.discount_details[0].discount_price * 100) / 100) +")"); 	 	    
...
{ //new-orders
	...
	//	icon_trans_type = getTransactionTypeIcons( val.trans_type_raw );
		...
		htm += '<div class="order-date"><b> ' + val.date_of_booking + ' ' + val.booking_time+'</b></div>';
		   htm+='<div class="order-status"> <span class="label status '+ val.status_raw +'"> ';
		   //htm+='<ons-icon icon="'+icons+'" class="icon '+icons2+'"></ons-icon>';
		   htm+=val.status;
		   htm+='</span></div>';
...
function displayBookingDetails(data)
{
	...
	html += TPLorderRow(getTrans("Available Seats", 'available_seats'), data.available_seats);
...
function bookingChangeStats()
{
	...
	//     onSuccess : function() {
	      ...
	      var info=getMerchantInfoStorage();
	      params+="&token="+getStorage("merchant_token");	       
	      if($.trim($('#bookAtableRemarks').val())!='')
		  {
		  	var remarks = $('#bookAtableRemarks').val();			  	 
