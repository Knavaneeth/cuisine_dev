...
class StoreController extends CController
{
	...
	public function actiontest_realex()
	{
		$realex_payment_json = require_once('../rxp-hpp/index.php');
		return $realex_payment_json;
		/* require_once('/rxp-hpp/vendor/autoload.php');
		...
		$hppRequest = (new HppRequest())
			->addMerchantId("bollywoodbytes")
			->addAccount("internet")
			->addAmount("1001")
			->addCurrency("EUR")
			->addAutoSettleFlag(TRUE);

		$realexHpp = new RealexHpp("R2TCgENWYr");
	...
	{
		$realex_response_json = require_once('../rxp-hpp/hpp_response.php');
		print_r($realex_response_json);
		print_r($realex_response_json->getResult());
	}
			...
			{				
				...
				{					
					...
					{						
						...
						$table_booked_stmt = "SELECT * FROM `mt_bookingtable` WHERE `date_booking` = '".$date_booking."' AND decliened = 1 AND `merchant_id` = ".$merchant_id;
	...
	{
		$this->render('Booking_confirmation');
	}
