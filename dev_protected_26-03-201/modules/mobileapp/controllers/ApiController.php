						...
						/*	$category_img = $this->get_category_image($val['item_category_id']);
							...
							{								 
								...
								{		
									...
									if(isset($category_img[0]['img_url'])&&!empty($category_img[0]['img_url']))
									{
										$category_img_url .= FunctionsV3::getFoodDefaultImage($category_img[0]['img_url'])."||";
									}									 
								}
	...
	public function actionMobileLoadMore()
	{
		...
		if($mtid!=''&&$category_id!=''&&$start!=''&&$end!='')
		{
			...
			if($resp=Yii::app()->functions->getItemByCategoryMobile($category_id,false,$mtid,$start,$end,$type="load_more"))
			{
				...
				{					 
					$category_details = json_decode($resp_value['item_category_id'],true);
					$category_img_url = '';	
					if(is_array($category_details)&&sizeof($category_details>0))
					{								 
						foreach ($category_details as $category_details_value) 
					...
					{
						$category_img = $this->get_category_image($resp_value['item_category_id']);
						if(isset($category_img[0]['img_url'])&&!empty($category_img[0]['img_url']))
						{
							$category_img_url = FunctionsV3::getFoodDefaultImage($category_img[0]['img_url']);
						}
					}
					$resp[$resp_key]['category_img_url'] = rtrim($category_img_url,"||") ;	 
