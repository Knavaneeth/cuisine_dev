...
*/ 
    ...
    public static function merchantOpenTag($merchant_id='')
    {
	    ...
	    $merchant_stores_late_extn  = '';

	     $now = new DateTime();
		// $now->setTimezone(new DateTimeZone('Europe/Jersey'));
		$now->setTimezone(new DateTimeZone('Pacific/Kiritimati'));
	     ...
	     if($mechant_opening_time = Yii::app()->functions->get_merchant_splitup_time($merchant_id))
	     {
	     	...
	     	{	 
				...
				if($value['option_name']=="stores_late_extn")
	     		{
	     			$merchant_stores_late_extn = json_decode(stripslashes($value['option_value']),true);
	     		} 
			 }
			 $stores_late_extn = false ;			  
			 if(is_array($merchant_stores_late_extn))
			 {
				 if(in_array($today,$merchant_stores_late_extn))
				 {				  
					$stores_late_extn = true ;
				 }
			 }
			 // print_r($merchant_evening_closed[$today]);
	     	...
	     	{	 	     		 
	     		...
	     		{	
	     			...
	     			{	     				 
	     			 if($opened_now = Yii::app()->functions->checkBetweenTime($current_time,$merchant_morning_open[$today],$merchant_morning_closed[$today],$stores_late_extn,$session_type=1))
	     			 {	     			 	
	     			 	
	     			 }
	     			 else
	     			 {	     			 	
	     			 	$opened_now = Yii::app()->functions->checkBetweenTime($current_time,$merchant_evening_open[$today],$merchant_evening_closed[$today],$stores_late_extn,$session_type=2);	
	     			 }	     			  
					 }     			 
	     		}
     		}
