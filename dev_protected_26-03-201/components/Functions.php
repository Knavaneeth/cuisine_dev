...
{	
	...
	public function get_merchant_splitup_time($merchant_id)
	{
		$DbExt=new DbExt;
		$res = '';
		$deals_query = "SELECT * FROM  `mt_option` WHERE merchant_id = ".$merchant_id." AND (`option_name`  LIKE  'stores_open_ends' OR `option_name` LIKE  'stores_open_starts' OR `option_name`  LIKE  'stores_open_pm_start' OR `option_name`  LIKE 'stores_open_pm_ends' OR `option_name`  LIKE 'stores_open_day' OR `option_name`  LIKE 'stores_late_extn' );   ";
...
/*					}
   ...
   {    	
			...
			if (!array_key_exists($full_booking_day,$business_hours)){
				...
				if (!empty($booking_time)){
					if (array_key_exists($full_booking_day,$business_hours)){						
						...
						$stores_late_extn=Yii::app()->functions->getOption("stores_late_extn",$merchant_id);
		
						$stores_late_extn=!empty($stores_late_extn)?(array)json_decode($stores_late_extn):false;					 

						switch($full_booking_day)
						{
						case "mon":
							$day_key = 0 ;
							$day = "monday";
						...
						{							 
							...
							if(in_array($days_array[$day_key],$stores_late_extn))
							{								 
								$merchant_stores_late_extn = true ;
							}
						...
						if (!Yii::app()->functions->checkBetweenTime($booking_time,$t1,$t2,$merchant_stores_late_extn,$session_type=1))
						{								 
							...
							{								
						        ...
						        if (Yii::app()->functions->checkBetweenTime($booking_time,$t1,$t2,$merchant_stores_late_extn,$session_type=2))
						        {						        	 
						        	return true;
						        } 
							}							
    ...
    public function checkBetweenTime($current_time='',$sunrise='',$sunset='',$merchant_stores_late_extn='',$session_type='')
    {    	
	    ...
	    // echo $current_time ."  ". $sunrise ."  ". $sunset." ".$merchant_stores_late_extn." ".$session_type."<br />" ;        	 
		...
		$begin = new DateTime(date("Y/m/d")." ".$sunrise);
		$end   = new DateTime(date("Y/m/d")." ".$sunset);
		 // echo $merchant_stores_late_extn."    ".$session_type;
		if($merchant_stores_late_extn==true&&$session_type==2)
		{	  
			$end = new DateTime(date('Y-m-d', strtotime(' + 1 day'))." ".$sunset);	  
			// print_r($end);
		} 		
		$now = new DateTime(date("Y/m/d")." ".$current_time);		 
		/* print_r($begin);
		print_r($end);
		print_r($now); */
