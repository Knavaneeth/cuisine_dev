...
function kcbCreateDevice_printer(devobj, retcode) {
	if (retcode == 'OK') {
		...
		kPrinter.onreceive = function (res) { //alert(res.success);
			...
			$(".kitchen-printer-status").css("color", "green");
		};
...
function print() {
	...
	if (kIPAddress != "" || kIPAddress != null) {
		ePosDev.deleteDevice('local_printer',function() {});
		//printer = null;
		InitKitchenPrinter();
	}
	setTimeout(function () {
...
function printToKitchen() {
	...
	if (!empty(data.item)) {
		$.each(data.item, function (key, val) {
			if (val.item_name != null) {
				...
				if (!empty(val.sub_item_new)) {
					kPrinter.addTextStyle(false, false, false, kPrinter.COLOR_2);
				...
				kPrinter.addTextStyle(false, false, false, kPrinter.COLOR_1);
			}
		}); // end of $.each loop
	}
	...
	kPrinter == null;
	kePosDev.deleteDevice('local_printer',function() {});
