...
$this->msg=t("We have sent bank information instruction to your email")." :$merchant_email";
	...
	public function parish_delivery_prices()
	{
		...
		if(isset($this->data['deliver_all_parish'])&&($this->data['deliver_all_parish']==2))
		{
			...
			if(isset($this->data['merchant_delivery_type'])&&($this->data['merchant_delivery_type']==0))
			{
				$this->data['minimum_order_req'] = 0;
				$this->data['free_delivery_above_price'] = 0;
				$this->data['delivery_fee'] = 0;
			}	
				...
				{				
					foreach ($this->data['service_available'] as $key => $parish_id) 
					{
						...
						{ */
							...
							$parish_free_above		 	= isset($this->data['parish_free_above_'.$key])?$this->data['parish_free_above_'.$key]:'';
							$services[$key] = array('delivery_fee'=>$delivery_fee,'chargable_type'=>$chargable_type,'minimum_order_req_parish'=>$minimum_order_req_parish,'parish_min_amt'=>$parish_min_amt,'parish_free_above'=>$parish_free_above);
						// }			 
					}				 
					$merchant_delivery_type =  isset($this->data['merchant_delivery_type'])?$this->data['merchant_delivery_type']:'';
					$minimum_order_req      =  isset($this->data['minimum_order_req'])?$this->data['minimum_order_req']:''; 	
