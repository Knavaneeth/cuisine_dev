...
public function displayOrderHTML($data = '', $cart_item = '', $receipt = false, $new_order_id = '')
{
	...
	if (isset($cart_item))
	{
		...
		<a href="javascript:;" class="edit_item" title="edit" data-row="' . $data_row . '" identity = "'.strtolower($item_array[$key]['size_words']).'" rel="' . $val['item_id'] . '" >
			...
			{
				$delivering_paish = isset($_SESSION['kr_item']['parish_delivery_rate']['delivering_paish'])?$_SESSION['kr_item']['parish_delivery_rate']['delivering_paish']:'';
				unset($_SESSION['kr_item']['parish_delivery_rate']);
				Yii::app()->functions->Default_address_parish_delivery($delivering_paish,$mid);
		...
		<input type="hidden" value="' .$discount_price. '" id="hidden_deals_discount" name="hidden_deals_discount" />
			...
			if ($data['delivery_type'] == "delivery")
			{
				if (!isset($_GET['backend']))
				{
					...
					if(isset($_SESSION['kr_item']['parish_delivery_rate']))
					{
						$free_delivery_above_price =	isset($_SESSION['kr_item']['parish_delivery_rate']['parish_free_above'])?$_SESSION['kr_item']['parish_delivery_rate']['parish_free_above']:'';
						// echo $free_delivery_above_price;
						if(trim($free_delivery_above_price)=='')
						{							 
							/* echo "<pre>";
							print_r($_SESSION['kr_item']['parish_delivery_rate']);
							echo "<pre>"; */
			...
			if(isset($_SESSION['kr_item']['parish_delivery_rate']))
			{
				$free_delivery_above = isset($_SESSION['kr_item']['parish_delivery_rate']['parish_free_above'])?$_SESSION['kr_item']['parish_delivery_rate']['parish_free_above']:0;
				$parish_min_amt = isset($_SESSION['kr_item']['parish_delivery_rate']['minimum_order'])?$_SESSION['kr_item']['parish_delivery_rate']['minimum_order']:0;
				$parish_min_amt_pretty = isset($_SESSION['kr_item']['parish_delivery_rate']['minimum_order'])?displayPrice(baseCurrency() , prettyFormat($_SESSION['kr_item']['parish_delivery_rate']['minimum_order'])):0;
			}
			...
			$this->details = array(
				'item-count' => $x,
				'html' => $htm,
				'parish_min_amt' => $parish_min_amt,
				'parish_min_amt_pretty'=>$parish_min_amt_pretty,
				'raw' => array(
					'item' => $item_array,
...
/*					}
	...
	public static function merchantOpenStatus($merchant_id='',$current_date)
    {
...
/*	public function checkAddonItemsExist($merchant_id='')
	...
	public function Default_address_parish_delivery_mobile($parish,$merchant_id)
	{
		...
		public function getMinimumOrder($merchant_id)
	{				
		$return_array = array(); 
		if($merchant_id!='')
		{	
			...
			{ 
				if(isset($res[0]['services'])&&!empty($res[0]['services']))
				{
					...
					{
						$return_array = array('deliver_multi_parish'=>true);
					} 
				}
				...
				if(isset($res[0]['deliver_to_all_parish'])&&($res[0]['deliver_to_all_parish']==2))
				{
					if(isset($res[0]['merchant_delivery_type'])&&($res[0]['merchant_delivery_type']==1))
					{
						if(isset($res[0]['minimum_order_req'])&&($res[0]['minimum_order_req']==2))
						{	
							$return_array = array('deliver_multi_parish'=>false,'minimum_order'=>$res[0]['minimum_order_amount']);							 
						}
					}
				}
			}	 
