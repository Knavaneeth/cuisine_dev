...
echo CHtml::hiddenField('minimum_order','');
echo CHtml::hiddenField('minimum_order_pretty','');
					...
					<div class="sidebar">
						...
						<!--<div class="est-delivery">
							...
							$exp = explode(' ',FunctionsV3::getDeliveryEstimation($merchant_id)); ?>
								<span class="est-duration"><?php echo isset($exp[0])?$exp[0]:''; ?></span><span> <?php echo isset($exp[1])?$exp[1]:''; ?></span>
							<?php }else{ ?>
						...
						<div class="est-delivery">
				            ...
				            {   
								...
								<div class="est-duration"><?php echo $deliver_time ; ?></div><div> <?php echo isset($exp[1])?$exp[1]:''; ?></div>
							<?php }else{ ?>
							<p><?php echo ucfirst(FunctionsV3::getDeliveryEstimation($merchant_id)); ?> <p>
							<?php } ?> 
						</div>
						...
						<div class="cart-box">
							<div class="choose-parish2">
								  <?php echo CHtml::dropDownList('parish',isset($_SESSION['kr_item']['parish_delivery_rate']['delivering_paish'])?$_SESSION['kr_item']['parish_delivery_rate']['delivering_paish']:'',
				  ...
				  <table class="table">
				    ...
				    <tbody>
				       <?php $parish_list = Yii::app()->functions->ParishListDropdown(); 
								...
								$parish_free_above	  =  isset($data['parish_free_above'])?$data['parish_free_above']:0;
				       		 	?>
				       		 <tr><td><?php print_r($parishes); ?></td><td><?php echo " £ " .number_format((float)$minimum_order_amount, 2, '.', ''); ?></td><td><?php echo " £ " .number_format((float)$delivery_fee, 2, '.', ''); ?></td> <td><?php echo " £ " .number_format((float) $parish_free_above, 2, '.', ''); ?></td> </tr>	
				       	<?php
				       } 
				       ?>
				    </tbody>
		    ...
		    <?php  
				$parish_list = Yii::app()->functions->ParishListMerchant(); 	       		 
       			...
       			{ 
					...
					$parish_free_above	  =  isset($value['parish_free_above'])?$value['parish_free_above']:0;
       				?>
       				 <tr><td><?php echo $parish_list[$parish_id]; ?></td><td><?php echo " £ " .number_format((float)$parish_min_amt, 2, '.', ''); ?></td><td><?php echo " £ " .number_format((float)$delivery_fee, 2, '.', ''); ?></td> <td><?php echo " £ " .number_format((float) $parish_free_above, 2, '.', ''); ?></td> </tr> 
       		<?php } ?>
       		      </tbody>
				  </table>
				</div>
