...
{	
    ...
    public static function sendEmailSMS($order_id='')
    {
		...
		{    		
			...
			$merchant_delivery_estimation = 0;
			if($merchant_delivery_estimation = Yii::app()->functions->getOption("merchant_delivery_estimation",$res['merchant_id']))
			{
    		...
    		if (is_array($tpl) && count($tpl)>=1){
				...
				if(($merchant_delivery_estimation>0)&&($res['status']=="accepted"))
				{					 
					$tpl['content'] .= " Note : Estimated Delivery Time will be ".$merchant_delivery_estimation." Mins";									
				}
    		...
    		/*SEND SMS*/
			$contact_phone = trim($res['contact_phone1']);    
			...
			{   
	    		...
	    		if (!empty($sms_tpl)){	    			
					...
					if(($merchant_delivery_estimation>0)&&($res['status']=="accepted"))
					{					 
						$sms_tpl .= " Note : Estimated Delivery Time will be ".$merchant_delivery_estimation." Mins";									
					}
