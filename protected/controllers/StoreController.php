...
class StoreController extends CController
{
	...
	{		 
		...
		if ($data = Yii::app()->functions->getOrder($_GET['order_id']))
		{
			...
			$curl = curl_init('http://dev.cuisine.je/rxp-hpp/index.php');
			$set_up = "dev";
			curl_setopt($curl, CURLOPT_POSTFIELDS,"chip_pin_user_id=".$chip_pin_user_id."&chip_pin_password=".$chip_pin_password."&chip_pin_SharedSecret=".$chip_pin_SharedSecret."&chip_pin_client_id=".$chip_pin_client_id."&total_amount=".$total_amount."&set_up=".$set_up);
	...
	public function actionRealexresponse()
	{
		...
		$curl = curl_init('http://dev.cuisine.je/rxp-hpp/hpp_response.php');
		...
		if($server_output['result']==00)
		{
			...
			if(isset($_SESSION['sendOrderEmail']))
			{						
				if($this->details['show_sms']==1)
				{
					/*send email to client and merchant*/
					AddonMobileApp::sendOrderEmail($_SESSION['sendOrderEmail']['cart'],$_SESSION['sendOrderEmail']['params'],$_SESSION['sendOrderEmail']['order_id'],$_SESSION['sendOrderEmail']['data'],$_SESSION['sendOrderEmail']['send_notification']);
					
					/*send sms to merchant and client*/
					AddonMobileApp::sendOrderSMS($_SESSION['sendOrderEmail']['cart'],$_SESSION['sendOrderEmail']['params'],$_SESSION['sendOrderEmail']['order_id'],$_SESSION['sendOrderEmail']['data'],$_SESSION['sendOrderEmail']['send_notification']);    
				}							
				unset($_SESSION['sendOrderEmail']);
			}	           	   
	...
	public function actionSelected_parish_delivery()
	{
		...
		if ( $res=$db_ext->rst($stmt))
		{
			if(isset($res[0]['services'])&&!empty($res[0]['services']))
			{
				...
				{
					 if(($value['chargable_type']==1)&&($value['minimum_order_req_parish'] == 2))
					 {
						if($key==$parish)
						{
							if(sizeof($_SESSION['kr_item'])>0)
							{							 
								$_SESSION['kr_item']['parish_delivery_rate'] = array('merchant_id'=>$_POST['merchant_id'],'minimum_order'=>$value['parish_min_amt'],'delivery_fee'=>$value['delivery_fee'],'parish_free_above'=>$value['parish_free_above'],'delivering_paish'=>$parish);							 
							}						
					 ...
					 {
						 $_SESSION['kr_item']['parish_delivery_rate'] = array('merchant_id'=>$_POST['merchant_id'],'minimum_order'=>0,'delivery_fee'=>0,'parish_free_above'=>0,'delivering_paish'=>$parish);							 
					 }
				}
			}
		...
		if ( $res=$db_ext->rst($stmt))
		{
				...
				foreach ($services as $key => $value) 
				{
					...
					{ 
						...
						if(sizeof($_SESSION['kr_item'])>0)
						{
							...
							{
								$_SESSION['kr_item']['parish_delivery_rate'] = array('merchant_id'=>$_POST['merchant_id'],'minimum_order'=>$value['parish_min_amt'],'delivery_fee'=>$value['delivery_fee'],'parish_free_above'=>$value['parish_free_above'],'delivering_paish'=>$parish);
								echo $delivery_place."|".$parish_id."|".$value['parish_min_amt'];
							}
							else 
							{
								 $_SESSION['kr_item']['parish_delivery_rate'] = array('merchant_id'=>$_POST['merchant_id'],'minimum_order'=>0,'delivery_fee'=>0,'parish_free_above'=>0,'delivering_paish'=>$parish);
								 echo $delivery_place."|".$parish_id."|0";
							}
			...
			//		{	
						if(sizeof($_SESSION['kr_item'])>0)
						{
							$delivery_place = "success";
							$_SESSION['kr_item']['parish_delivery_rate'] = array('merchant_id'=>$_POST['merchant_id'],'minimum_order'=>$res[0]['minimum_order_amount'],'delivery_fee'=>$res[0]['delivery_fee']);	
							echo $delivery_place."|".$parish_id."|".$res[0]['minimum_order_amount'];
						}
			//		}
			//	}
			}
